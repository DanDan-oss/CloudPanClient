...
function build_server
{
    ...
    [ -d "${server_code_root}/fdfs_register/build" ] && rm -rf ${server_code_root}/fdfs_register/build
    [ -d "${server_code_root}/fdfs_register/bin" ] && rm -rf ${server_code_root}/fdfs_register/bin
    mkdir -p ${server_code_root}/fdfs_register/build
    cd ${server_code_root}/fdfs_register/build
    cmake ..  && make
    cp -rf ../bin/fdfs_register.exe ..

    echo "build fdfs_login"
    [ -d "${server_code_root}/fdfs_login/build" ] && rm -rf ${server_code_root}/fdfs_login/build
    [ -d "${server_code_root}/fdfs_login/bin" ] && rm -rf ${server_code_root}/fdfs_login/bin
    mkdir -p ${server_code_root}/fdfs_login/build
    cd ${server_code_root}/fdfs_login/build
    cmake ..  && make
    cp -rf ../bin/fdfs_login.exe ..
...
function fdfs_srever_stop
{
    ...
    fdfs_login_pid=$(ps aux | grep "fdfs_login.exe" | grep -v grep | awk '{print $2}')
...
function fdfs_srever_start
{
    ...
    spawn-fcgi -a 127.0.0.1 -p 7791 -f ${CLOUDPAN_PATH}/code/http_server/fdfs_login/fdfs_login.exe
