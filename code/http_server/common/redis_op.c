...
int rop_selectdatabase(redisContext *conn, unsigned int db_no)
{
    ...
    if (reply == NULL)
    {
        ...
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]Select database %d error!%s\n", db_no, conn->errstr);
        retn = -1;
        goto END;
    }

    printf("[+][GMS_REDIS]Select database %d SUCCESS!\n", db_no);
    LOG_INFO(REDIS_LOG_MODULE, "[+][GMS_REDIS]Select database %d SUCCESS!\n", db_no);
...
int rop_flush_database(redisContext *conn)
{
    ...
    if (reply == NULL)
    {
        ...
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]Clear all data error\n");
        retn = -1;
        goto END;
    }

    printf("[+][GMS_REDIS]Clear all data!!\n");
    LOG_INFO(REDIS_LOG_MODULE, "[+][GMS_REDIS]Clear all data!!\n");
...
int rop_is_key_exist(redisContext *conn, char *key)
{
    ...
    if (reply->type != REDIS_REPLY_INTEGER)
    {
        ...
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]is key exist get wrong type! %s\n", conn->errstr);
        retn = -1;
        goto END;
    }
...
int rop_del_key(redisContext *conn, char *key)
{
    ...
    if (reply->type != REDIS_REPLY_INTEGER)
    {
        ...
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS] DEL key %s ERROR %s\n", key, conn->errstr);
        retn = -1;
        goto END;
    }
...
int rop_set_key_lifecycle(redisContext *conn, char *key, time_t delete_time)
{
    ...
    if (reply->type != REDIS_REPLY_INTEGER)
    {
        ...
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]Set key:%s delete time ERROR! %s\n", key, conn->errstr);
        retn = -1;
    }
    if (reply->integer == 1)
    {
...
void rop_show_keys(redisContext *conn, char *pattern)
{
    ...
    if (reply->type != REDIS_REPLY_ARRAY)
    {
        ...
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]show all keys and data wrong type! %s\n", conn->errstr);
        goto END;
    }
...
int rop_redis_append(redisContext *conn, RCOMMANDS cmds, int cmd_num)
{
    ...
    for (i = 0; i < cmd_num; ++i)
    {
        ...
        if (retn != REDIS_OK)
        {
            ...
            LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]Append Command: %s ERROR! %s\n", cmds[i], conn->errstr);
            retn = -1;
            goto END;
        }
        retn = 0;
    ...
    for (i = 0; i < cmd_num; ++i)
    {
        ...
        if (retn != REDIS_OK)
        {
            ...
            LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]Commit Command:%s ERROR! %s\n", cmds[i], conn->errstr);
            freeReplyObject(reply);
            break;
        }
        freeReplyObject(reply);
...
int rop_redis_command(redisContext *conn, char *cmd)
{
    ...
    if (reply == NULL)
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]Command : %s ERROR!%s\n", cmd, conn->errstr);
        retn = -1;
    }
...
void rop_test_reply_type(redisReply *reply)
{
    ...
    case REDIS_REPLY_STATUS:
        LOG_INFO(REDIS_LOG_MODULE, "[+][GMS_REDIS]=REDIS_REPLY_STATUS=[string] use reply->str to get data, reply->len get data len\n");
        break;
    case REDIS_REPLY_ERROR:
        LOG_INFO(REDIS_LOG_MODULE, "[+][GMS_REDIS]=REDIS_REPLY_ERROR=[string] use reply->str to get data, reply->len get date len\n");
        break;
    case REDIS_REPLY_INTEGER:
        LOG_INFO(REDIS_LOG_MODULE, "[+][GMS_REDIS]=REDIS_REPLY_INTEGER=[long long] use reply->integer to get data\n");
        break;
    case REDIS_REPLY_NIL:
        LOG_INFO(REDIS_LOG_MODULE, "[+][GMS_REDIS]=REDIS_REPLY_NIL=[] data not exist\n");
        break;
    case REDIS_REPLY_ARRAY:
        LOG_INFO(REDIS_LOG_MODULE, "[+][GMS_REDIS]=REDIS_REPLY_ARRAY=[array] use reply->elements to get number of data, reply->element[index] to get (struct redisReply*) Object\n");
        break;
    case REDIS_REPLY_STRING:
        LOG_INFO(REDIS_LOG_MODULE, "[+][GMS_REDIS]=REDIS_REPLY_string=[string] use reply->str to get data, reply->len get data len\n");
        break;
    default:
        LOG_INFO(REDIS_LOG_MODULE, "[-][GMS_REDIS]Can't parse this type\n");
        break;
    }
}
...
redisContext *rop_connectdb_nopwd(char *ip_str, char *port_str)
{
    ...
    if (conn == NULL)
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]Connect %s:%d Error:Can't allocate redis context!\n", ip_str, port);
        goto END;
    }

    if (conn->err)
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]Connect %s:%d Error:%s\n", ip_str, port, conn->errstr);
        redisFree(conn);
        conn = NULL;
        goto END;
    }

    LOG_INFO(REDIS_LOG_MODULE, "[+][GMS_REDIS]Connect %s:%d SUCCESS!\n", ip_str, port);
...
redisContext *rop_connectdb(char *ip_str, char *port_str, char *pwd)
{
    ...
    if (reply == NULL)
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]Command : auth %s ERROR!\n", pwd);
        conn = NULL;
        goto END;
    }
    freeReplyObject(reply);
...
redisContext *rop_connectdb_unix(char *sock_path, char *pwd)
{
    ...
    if (conn == NULL)
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]Connect domain-unix:%s Error:Can't allocate redis context!\n", sock_path);
        goto END;
    }

    if (conn->err)
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]Connect domain-unix:%s Error:%s\n", sock_path, conn->errstr);
        redisFree(conn);
        conn = NULL;
        goto END;
    }
    ...
    LOG_INFO(REDIS_LOG_MODULE, "[+][GMS_REDIS]Connect domain-unix:%s SUCCESS!\n", sock_path);
...
void rop_disconnect(redisContext *conn)
{
    ...
    LOG_INFO(REDIS_LOG_MODULE, "[+][GMS_REDIS]Disconnect SUCCESS!\n");
}
...
int rop_hash_set_append(redisContext *conn, char *key, RFIELDS fields, RVALUES values, int val_num)
{
    ...
    for (i = 0; i < val_num; ++i)
    {
        ...
        if (retn != REDIS_OK)
        {
            LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]HSET %s %s %s ERROR![%s]\n", key, fields[i], values[i], conn->errstr);
            retn = -1;
            goto END;
        }
        retn = 0;
    ...
    for (i = 0; i < val_num; ++i)
    {
        ...
        {
            retn = -1;
            LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]Commit HSET %s %s %s ERROR![%s]\n", key, fields[i], values[i], conn->errstr);
            freeReplyObject(reply);
            break;
        }
        freeReplyObject(reply);
...
int rop_hash_get(redisContext *conn, char *key, char *field, char *value)
{
    ...
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]hget %s %s  error %s\n", key, field, conn->errstr);
        retn = -1;
        goto END;
    }
...
int rop_hash_del(redisContext *conn, char *key, char *field)
{
    ...
    if (reply->integer != 1)
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]hdel %s %s %s error %s\n", key, field, conn->errstr);
        retn = -1;
        goto END;
    }
                                     ...
                                     RVALUES values)
{
    ...
    if (cmd == NULL)
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]create hash table %s error\n", key);
        retn = -1;
    ...
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]Create hash table %s Error:%s,%s\n", key, reply->str, conn->errstr);
...
int rop_hincrement_one_field(redisContext *conn, char *key, char *field, unsigned int num)
{
    ...
    if (reply == NULL)
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]increment %s %s error %s\n", key, field, conn->errstr);
        retn = -1;
        goto END;
    }
...
int rop_list_push_append(redisContext *conn, char *key, RVALUES values, int val_num)
{
    ...
    for (i = 0; i < val_num; ++i)
    {
        ...
        if (retn != REDIS_OK)
        {
            LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]PLUSH %s %s ERROR! %s\n", key, values[i], conn->errstr);
            retn = -1;
            goto END;
        }
        retn = 0;
    ...
    for (i = 0; i < val_num; ++i)
    {
        ...
        {
            retn = -1;
            LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]Commit LPUSH %s %s ERROR! %s\n", key, values[i], conn->errstr);
            freeReplyObject(reply);
            break;
        }
        freeReplyObject(reply);
...
int rop_list_push(redisContext *conn, char *key, char *value)
{
    ...
    if (reply->type != REDIS_REPLY_INTEGER)
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]LPUSH %s %s error!%s\n", key, value, conn->errstr);
        retn = -1;
    }
...
int rop_get_list_cnt(redisContext *conn, char *key)
{
    ...
    if (reply->type != REDIS_REPLY_INTEGER)
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]LLEN %s error %s\n", key, conn->errstr);
        cnt = -1;
        goto END;
    }
...
int rop_trim_list(redisContext *conn, char *key, int begin, int end)
{
    ...
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]LTRIM %s %d %d error!%s\n", key, begin, end, conn->errstr);
        retn = -1;
    }
...
int rop_range_list(redisContext *conn, char *key, int from_pos, int end_pos, RVALUES values, int *get_num)
{
    ...
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]LRANGE %s  error!%s\n", key, conn->errstr);
        retn = -1;
        goto END;
    }
...
int rop_zset_add(redisContext *conn, char *key, long score, char *member)
{
    ...
    if (reply->integer != 1)
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]ZADD: %s,member: %s Error:%s,%s\n", key, member, reply->str, conn->errstr);
        retn = -1;
        goto END;
    }
...
int rop_zset_zrem(redisContext *conn, char *key, char *member)
{
    ...
    if (reply->integer != 1)
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]ZREM: %s,member: %s Error:%s,%s\n", key, member, reply->str, conn->errstr);
        retn = -1;
        goto END;
    }
...
int rop_zset_del_all(redisContext *conn, char *key)
{
    ...
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]ZREMRANGEBYRANK: %s,Error:%s,%s\n", key, reply->str, conn->errstr);
        retn = -1;
        goto END;
    }
...
int rop_zset_zrevrange(redisContext *conn, char *key, int from_pos, int end_pos, RVALUES values, int *get_num)
{
    ...
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]ZREVRANGE %s  error!%s\n", key, conn->errstr);
        retn = -1;
        goto END;
    }
...
int rop_zset_increment(redisContext *conn, char *key, char *member)
{
    ...
    if (strcmp(reply->str, "OK") != 0)
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]Add or increment table: %s,member: %s Error:%s,%s\n", key, member, reply->str, conn->errstr);
...
int rop_zset_zcard(redisContext *conn, char *key)
{
    ...
    if (reply->type != REDIS_REPLY_INTEGER)
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]ZCARD %s error %s\n", key, conn->errstr);
        cnt = -1;
        goto END;
    }
...
int rop_zset_get_score(redisContext *conn, char *key, char *member)
{
    ...
    if (reply->type != REDIS_REPLY_STRING)
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]ZSCORE %s %s error %s\n", key, member, conn->errstr);
        score = -1;
        goto END;
    }
...
int rop_zset_exit(redisContext *conn, char *key, char *member)
{
    ...
    if (reply->type != REDIS_REPLY_INTEGER)
    {
        LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]zlexcount: %s,member: %s Error:%s,%s\n", key, member, reply->str, conn->errstr);
        retn = -1;
        goto END;
    }
...
int rop_zset_increment_append(redisContext *conn, char *key, RVALUES values, int val_num)
{
    ...
    for (i = 0; i < val_num; ++i)
    {
        ...
        if (retn != REDIS_OK)
        {
            LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]ZINCRBY %s 1 %s ERROR! %s\n", key, values[i], conn->errstr);
            retn = -1;
            goto END;
        }
        retn = 0;
    ...
    for (i = 0; i < val_num; ++i)
    {
        ...
        {
            retn = -1;
            LOG_ERROR(REDIS_LOG_MODULE, "[-][GMS_REDIS]Commit ZINCRBY %s 1 %s ERROR!%s\n", key, values[i], conn->errstr);
            freeReplyObject(reply);
            break;
        }
        freeReplyObject(reply);
